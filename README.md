...
- `Seance.Channel`: A behavior module for creating Phoenix channels that handle Seance logic.
- `Seance.Encoder`: A protocol for customizing how data is encoded before being sent to clients.
- `Seance.Event`: A struct representing events that can be sent from the server to clients.
- `Seance.MessageBuilder`: A module for creating state change and patch messages.
...
1. Add Seance to your dependencies in `mix.exs`:
...
2. Create a channel using `Seance.Channel`:

   ```elixir
   defmodule MyAppWeb.MyChannel do
     use Seance.Channel, web_module: MyAppWeb
...
- **Custom Encoders**: Implement the `Seance.Encoder` protocol to customize how data is serialized before being sent to clients.

## Testing

The library includes `Seance.TestHelpers` module for writing tests for your Seance channels.

## Using with React

Seance integrates seamlessly with React using mobx-react-lite for efficient rendering and state management. Here's an example of how to use Seance in a React component:
