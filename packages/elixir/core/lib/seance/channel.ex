defmodule Seance.Channel do
  @moduledoc """
  To build a Seance application, you'll first want to add a channel that `use`s this module.

  ```
    use Seance.Channel, web_module: MyAppWeb, max_version: 100
  ```
  - `message_builder` optional, defaults to `{Seance.MessageBuilder, ignore_keys: [:__meta__]}`. If
  ...
  See `Seance.MessageBuilder` for details
  ...
  can contain a single `Seance.Event` or a list of events as well as the new state.
  """
  @callback handle_event(event_name :: binary(), payload :: term(), state :: term()) ::
              {:reply, reply :: %Seance.Event{} | list(%Seance.Event{}),
               new_state :: any()}
  ...
  you need to send events to the client, otherwise return `:noreply`.  `:reply` tuples
  can contain a single `Seance.Event` or a list of events, as well as the new state and
    ...
    quote do
      ...
      @message_builder unquote(
                         Keyword.get(
                           opts,
                           :message_builder,
                           {Seance.MessageBuilder, ignore_keys: [:__meta__]}
                         )
                       )
      ...
      def handle_in("seance:event:" <> event_name, payload, %{assigns: assigns} = socket) do
